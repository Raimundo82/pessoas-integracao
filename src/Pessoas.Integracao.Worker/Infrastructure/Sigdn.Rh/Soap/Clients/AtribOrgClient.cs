    ...
    ) : IAtribOrgClient
{
    ...
    public async Task<Dictionary<PessoaImportKey, ZhrSAtribOrgOutput?>> GetAtribOrgAsync(IReadOnlyList<PessoaImportKey> importKey, CancellationToken cancellationToken)
    {
        ...
        var response = await channel.ZhrWsAtribOrgAsync(new ZhrWsAtribOrgRequest
        {
            ZhrWsAtribOrg = new ZhrWsAtribOrg { Input = [.. input] }
        });

        var output = response.ZhrWsAtribOrgResponse.Output;
